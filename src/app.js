...
import { baseNumber, parseNumDen, parseSubset } from './domain/numbers.js';
...
function initApp(){
  // charge la map FR -> setId au démarrage
  loadFrMap();
  ...
  if(serie) serie.onchange=()=>render();
  if(view) view.onchange=()=>render();
  ...
  if(minG) minG.oninput=()=>{ if(minG_label) minG_label.textContent=minG.value; render(); }
  ...
  function priceFromEntry(entry, variant){
    ...
    if(variant === 'reverse'){
      ...
      return rr > 0 ? rr : base; // fallback reverse -> normal
    }
    return base;
  }
  // Personnalisation possible côté app : window.ALT_PRICE_BY_SET = { sv7:{mode:'reverse'}, sv3pt5:{mode:'multiplier',factor:2} }
  function altPriceFromEntry(entry, setId){
    let val = priceFromEntry(entry,'reverse'); // défaut = même prix que reverse (ou normal en fallback)
  ...
  // ===== Valeur / Top 10 par série (N / Reverse / Alternative) =====
  async function computePriceStatsForSeries(slug){
    ...
    for(const r of rows){
      ...
      // Set (unique) : 1 de chaque version existante
